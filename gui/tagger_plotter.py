    ...
    def plot_wavenumbers(self, new_data, selected_channels = [1, 2, 3, 4]):
        ...
        for i, channel in enumerate(selected_channels):
            if f"wn_{channel}" in self.historical_data.columns and self.historical_data[f"wn_{channel}"].mean() > 0:
                fig.add_trace(go.Scatter(x=self.historical_data["timestamp"], y=self.historical_data[f"wn_{channel}"], mode="lines", name=f"WN_{channel}", line=dict(color=colors[i])))
...
app.layout = dbc.Container([
    dbc.Row([
        dbc.Col([
            html.H1("Tagger Monitor", style={'textAlign': 'center', 'marginBottom': '20px'}),
