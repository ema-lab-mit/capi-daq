...
class TaggerVisualizer:
    ...
    def events_per_wavelength(self, data):
        try:
            wavelength_counts = data.groupby('wavenumber').size().reset_index(name='counts')
            fig = px.line(wavelength_counts, x='wavenumber', y='counts',
                          title="Events per Wavelength",
                          labels={"wavenumber": "Wavenumber", "counts": "Number of Events"})
            fig.update_layout(showlegend=False, xaxis_title="Wavenumber", yaxis_title="Number of Events")
            return fig
        except Exception as e:
            logging.error(f"Error creating events per wavelength plot: {e}")
            st.error("Error creating events per wavelength plot.")
            return go.Figure()
...
def main(predefine_data_path=None):
    ...
    col5 = st.columns(1)
    events_per_wavelength_plot = col5[0].empty()
        ...
        try:
            ...
            if df.empty:
                ...
                **Time Since Last Bunch:**
                ...
                events_per_wavelength_plot.plotly_chart(viz.events_per_wavelength(df), use_container_width=True)
