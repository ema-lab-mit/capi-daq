...
default_settings = {
    ...
    "tof_hist_min": 1e-6,
    "tof_hist_max": 20e-6,
    "plot_rolling_window": 100,  # Default rolling window size
...
class PlotGenerator:
    def __init__(self, settings_dict: dict = default_settings):
        ...
        self.tof_hist_min = settings_dict.get("tof_hist_min", 1)
    ...
    def plot_events_over_time(self, max_points=MAX_POINTS_FOR_PLOT, yaxis_range=None, show_rolling_average=False, rolling_window_size=100):
        ...
        # Add rolling average line if enabled
        if show_rolling_average and rolling_window_size > 1:
            ...
            fig.add_trace(go.Scatter(
                ...
                y=rolling_avg,
                mode="lines",
                name=f"Rolling Average ({rolling_window_size} pts)",
                line=dict(color="red")
            ))

        fig.update_layout(
            xaxis_title="Monitoring Time (s)",
            yaxis_title="Total Events per Second",
    ...
    def plot_voltage(self, max_points=500):
        ...
        if "voltage" in self.historical_data.columns and self.historical_data["voltage"].mean() >= 0:
            ...
            decimated_ts = delta_ts[-len(decimated_data):]
    ...
    dbc.Modal(
        [
            ...
            dbc.ModalBody([
                ...
                dbc.Checklist(
                    options=[
                        {"label": "Show Rolling Average", "value": "show_rolling_average"},
                    ],
                ...
                dcc.Input(id='events-rolling-window-size', type='number', value=default_settings['plot_rolling_window'], min=1),
                html.Br(),
...
def update_plots(n_intervals, clear_clicks, update_histogram_clicks, wavenumbers_channels, events_ymin, events_ymax, show_rolling_average_values, events_rolling_window_size, events_relayout_data, tof_relayout_data, wavenumbers_relayout_data):
    try:
        ...
        try:
            ...
            # Ensure rolling window size is valid
            if events_rolling_window_size is None or events_rolling_window_size <= 0:
